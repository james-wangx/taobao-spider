...
from setting import COOKIES_PATH, PRODUCT, SESSION, logger
...
class Login:
    ...
    def _html(self):
        ...
        response = SESSION.get(
            url=self.login_url, headers=self.headers, verify=False)
    ...
    def verify_login(self, response):
        ...
        :return:
        """
        iframe_url = response.json()['content']['data']['iframeRedirectUrl']
        iframe_response = SESSION.get(
            url=iframe_url, headers=self.headers, verify=False)

        remote_url = re.search(r'<iframe src="(.*?)"',
                               iframe_response.text).group(1)
        remote_response = SESSION.get(
            url=remote_url, headers=self.headers, verify=False)

        normal_url = remote_response.url
        normal_response = SESSION.get(
            url=normal_url, headers=self.headers, verify=False)

        verify_url = re.search(
            r'window.location.href = "(.*?)"', normal_response.text).group(1) + "1"
        verify_response = SESSION.get(
            url=verify_url, headers=self.headers, verify=False)

        safe_login_url = re.search(
            r'window.location.href = "(.*?)"', verify_response.text).group(1)
        safe_login_response = SESSION.get(
            url=safe_login_url, headers=self.headers, verify=False)

        self.redirect_url = re.search(
            r'"redirectUrl":"(.*?)"', safe_login_response.text).group(1)
    ...
    def logged(self):
        ...
        try:
            response = SESSION.post(
                url=self.commit_url, headers=headers, data=post_data, verify=False)
            response.raise_for_status()
        except Exception as e:
            logger.error(f'登录失败，原因：')
            raise e
        self.queue_cookies()
        if 'iframeRedirectUrl' in response.json()['content']['data']:
            logger.warning('需要安全验证')
            self.verify_login(response)
        else:
            self.redirect_url = response.json(
    ...
    def print_title(self):
        ...
        """
        try:
            response = SESSION.get(url=self.redirect_url,
    ...
    def parse_detail(self):
        ...
        self.result = [dict(zip(keys, value))
                       for value in match if len(value[4]) < 50]
